 ...
 * - Supports canceling requests, confirming job completion, and editing/rescheduling via navigation to /log-technical-callout.
 ...
 * - Removed Edit Description button and functionality; renamed Reschedule to "Reschedule / Edit Active Job" (V1.34).
 * - Added focus management to confirm completion dialog to fix aria-hidden warning (V1.34).
 */
...
const CustomerDashboard: React.FC = () => {
  ...
  useEffect(() => {
    if (confirmingRequestId && dialogRef.current) {
  ...
  const fetchData = async () => {
    try {
      console.log(`Fetching customer profile for customerId: ${customerProfile?.id ?? 'unknown'}`);
      console.log(`Fetching requests for customerId: ${customerProfile?.id ?? 'unknown'}`);
  ...
  const handleCancelRequest = async (requestId: number) => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/requests/${requestId}`, {
        ...
        body: JSON.stringify({ customerId: customerProfile.id }),
        credentials: 'include'
      });
  ...
  const handleConfirmCompleteRequest = async () => {
    if (confirmingRequestId && customerProfile?.id) {
      try {
  ...
  return (
    <ErrorBoundary>
      <LocalizationProvider dateAdapter={AdapterDateFns}>
        <Container maxWidth="md" sx={{ py: 8, background: 'linear-gradient(to right, #1f2937, #111827)', minHeight: '100vh' }}>
